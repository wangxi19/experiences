/*
    use gcc -static flag to build a executable file rather than a shared object file
    in the parameter <program>
**
*/
...
int main(int argc, char **argv, char **envp)
{
    ...
    char * args[2];
    if (argc < 3) {
        printf("Usage: %s <program> <function>\nuse gcc -static flag to build executable file<program>\n", argv[0]);
        exit(0);
    }
    ...
    if (h.mem[0] != 0x7f || memcmp((char *)&h.mem[1], "ELF", 3)) {
